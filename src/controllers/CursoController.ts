  ...
  try {
    ...
    await AppDataSource.transaction(async (transactionalEntityManager) => {
      const cursoEstudianteRepository =
        transactionalEntityManager.getRepository(CursoEstudiante); //estudiantes cursando
      const cursoRepository = transactionalEntityManager.getRepository(Curso);

      const estudiantesAsignados = await cursoEstudianteRepository.count({
        where: { curso: { id: Number(id) } },
      });
      console.log(
        `Número de estudiantes cursando el curso: ${estudiantesAsignados}`
      );
      ...
      console.log(
        `Resultado de la eliminación: ${JSON.stringify(deleteResult)}`
      );
