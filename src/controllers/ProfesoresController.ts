...
export const validar = () => [
  ...
  check("profesion")
    .notEmpty()
    .withMessage("La profesion es obligatoria")
    .isLength({ min: 3 })
    .withMessage("La profesion debe tener al menos 3 caracteres"),
  ...
  try {
    ...
    await AppDataSource.transaction(async (transactionalEntityManager) => {
      ...
      if (cursosRelacionados > 0) {
        throw new Error(
          "El profesor tiene cursos a su cargo, no es posible eliminar"
        );
      }
